...
import { useStore, deleteRecord, downloadRecord, type RecordType } from "@/lib/mock-store";
            ...
            <TableBody>
              {filtered.map((r) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell><span className="text-xs rounded-full bg-accent/30 px-2 py-0.5">{r.status}</span></TableCell>
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-1">
                      <Button asChild size="icon" variant="ghost"><Link to="/patient/records/$id" params={{ id: r.id }}><Eye className="h-4 w-4" /></Link></Button>
