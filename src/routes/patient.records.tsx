...
function RecordsPage() {
  ...
  const handleDownload = async (id: string, cid: string, fileName: string) => {
    if (!isRealCid(cid)) { toast.error("No IPFS file attached to this record."); return; }
    setDownloading(id);
    try {
                ...
                <TableRow key={r.id}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-1">
                      ...
                      {/* Open in IPFS */}
                      {isRealCid(r.ipfsCid ?? "") && (
                        <Button size="icon" variant="ghost" title="Open in IPFS"
                          onClick={() => { try { openIpfsFile(r.ipfsCid!); } catch (err: unknown) { toast.error((err as Error).message); } }}>
                          <ExternalLink className="h-4 w-4" />
                        </Button>
                      )}

                      {/* Download */}
                      <Button size="icon" variant="ghost" title="Download" disabled={downloading === r.id}
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>This removes the record from your local list.</AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            <AlertDialogCancel>Cancel</AlertDialogCancel>
                            <AlertDialogAction onClick={() => { deleteRecord(r.id); toast.success("Record deleted"); }}>Delete</AlertDialogAction>
                          </AlertDialogFooter>
                        </AlertDialogContent>
                      </AlertDialog>
                    </div>
