...
function RequestPage() {
  ...
  return (
    <div>
      <PageHeader title="Request Access" description="Ask a patient to share their medical records with you." />
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <Card className="glass p-6">
          ...
          <Table>
            <TableHeader><TableRow><TableHead>Patient</TableHead><TableHead>Date</TableHead><TableHead>Status</TableHead></TableRow></TableHeader>
            <TableBody>
              {requests.map((r) => (
                <TableRow key={r.id}>
                  <TableCell className="font-mono text-xs">{shortAddr(r.patientAddress)}</TableCell>
                  <TableCell>{r.requestDate}</TableCell>
                  <TableCell><span className={`text-xs rounded-full px-2 py-0.5 ${r.status === "Approved" ? "bg-accent/30" : r.status === "Pending" ? "bg-chart-4/30" : "bg-destructive/20 text-destructive"}`}>{r.status}</span></TableCell>
                </TableRow>
              ))}
              {requests.length === 0 && <TableRow><TableCell colSpan={3} className="text-center text-muted-foreground py-6">No requests yet.</TableCell></TableRow>}
            </TableBody>
          </Table>
        </Card>
      </div>
